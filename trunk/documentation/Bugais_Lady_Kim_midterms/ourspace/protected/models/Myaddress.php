...
class Myaddress extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('firstname, middlename, lastname, gender, created_at, homeaddress, landline, cellphone', 'required'),
			array('firstname, middlename, lastname', 'length', 'max'=>30),
			array('gender', 'length', 'max'=>1),
			array('homeaddress', 'length', 'max'=>50),
			array('landline, cellphone', 'length', 'max'=>20),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, firstname, middlename, lastname, gender, created_at, homeaddress, landline, cellphone', 'safe', 'on'=>'search'),
		);
	}
